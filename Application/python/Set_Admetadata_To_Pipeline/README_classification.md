...
2. Set classification results from admetadata.
3. Set stream data by using plugin **classifier_sample** in application program.

## Essential knowledge
1. Understand plugin sample codes: **classifier_sample.py**.
...
The classifier_sample is the plugin that generated a random inferenced classification results to metadata.
...
Second, add plugin **classifier_sample** to the pipeline. This element will generate random classification result of each frame:
...
First, copy the built plugin **libadsetclassification.so** file to the plugin folder EVA installed, here used EVA_ROOT to preset the installed path of EVASDK. Then, go to the folder of the binary and run binary in terminal or cmd:
